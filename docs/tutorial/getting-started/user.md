...
2. Click **Next**
3. Choose a [previously](/tutorial/getting-started/module) created Group :
   - **TRN_SUPER_ADMIN** _if exists, this should be selected by default_
   ![](img/user/group.png)
4. Click **Next**
5. Create a Responsibility, click **Next**
   ![](img/user/resp.png)
   > For more information about Domains, see the [Responsibility](/make/usersrights/responsibilities) section.
6. Choose a new group :
   - For now, select **no**
7. Click **Next**

:::success

The **usertest** User is created and opened
...
4. Log-in using **usertest** :
   - Login : **usertest**
   - Password : _previously reset password_
5. Click **Connection**
...
Before moving on, and to make testing / configuration easier, we will add the `designer`
(_or the user used to design the application_) user to the `TRN_SUPERADMIN` Group.
...
5. Empty the platform's cache :
   - Via the header menu :
     ...
     - Click **Clear all sessions and all server caches**
       ![](img/user/clear-cache.png)
   - Via the keyboard shortcut : [ <kbd>Alt</kbd>+<kbd>C</kbd>+<kbd>C</kbd> ]
6. Log back in using `designer` (_or the user used to design the application_)

:::tip[Success]

**Expected result**:

- _My App_ Menu is visible
- The list of Suppliers is displayed when the menu is clicked
...
_Authentication error_ when logging in, check that:
