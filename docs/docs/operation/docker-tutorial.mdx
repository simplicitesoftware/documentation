...
sudo dnf -y install epel-release && sudo dnf -y install htop && sudo dnf clean all
```
</TabItem>
<TabItem value="centos" label="CentOS 7">
```shell
sudo yum -y install vim-enhanced git wget curl zip unzip && sudo yum clean all
sudo yum -y install epel-release && sudo yum -y install htop && sudo yum clean all
```
</TabItem>
</Tabs>
...
Note that you **may** also need to do a `loginctl enable-linger`
...
If you don't use the right port for the right usage (typically if you misuse the `8080` for an HTTPS endpoint of the reverse proxy)
it **will** result in unexpected behaviors
...
1. Install `cerbot` CLI:

```bash
sudo dnf -y install epel-release && sudo dnf -y install certbot && sudo dnf clean all
```
2. Add this to the HTTP server config in `nginx.conf` (before the `location / {...}` block):
...
3. Comment the HTTPS config in the `nginx.conf` file.

>4. Create a `.well-known` dir:

   ```shell
   mkdir .well-known
   ```

5. Add a new volume mapping to the `simplicite` service in the `docker-compose.yml` file:
...
7. Request the LetEncrypt certificate by:

   ```shell
   sudo certbot certonly --webroot -w . -d <myhostname>
   ```

8. Uncomment the HTTPS config  in the `nginx.conf` file.

9. Change the certificate and key volume mappings in the `docker-compose.yml` file to:
