...
See documentation to migrate your specific V3 components to V4 (See [this page](https://getbootstrap.com/4.0/migration)).
...
- See [this page](https://fullcalendar.io/upgrading-from-v3) to upgrade your specific usage before migrating to FullCalendar v4.
...
- New Websocket event `clearCache` used during a partial clear cache to clean the designer's UI, by removing
  client-side cache to force a full reload of object metadata.
...
- Min/max rows per object: new properties to override the default min/max of user's profile
  (when greater than 0, if not set the list will use the profile values)
...
- Allows to load on-the-fly external **JDBC driver** to connect external database:
  ...
  - or through an external jar loaded in one isolated classloader:
...
- New front APIs to change the page sizing:
  ...
  - `$ui.view.compact(ctn, enable)` to apply a compact mode to a container (default = body)
    ...
    - can be changed through the top level class `.compact`
...
- or articles in a new view item or through a shortcut
- or horizontal ticker in the footer (reloaded every 5 minutes)
- or on demand by news administrator to push a popup to granted users (through websocket only)
...
- Added a `hidden` mode to open a business model in a silent mode to edit the SVG through front JavaScript
- Optimization: avoid double calls to `initRefSelect` when user is picking a reference
- Crosstab cell meta-data ([See this post](http://community.simplicite.io/t/ajout-de-metadata-dans-un-tableau-croise/2459))
...
- Change user's language:
  - Switch easily the current language through user's profile in header
...
- Visible **Shortcuts**
  ...
  - `Plus` shortcuts are only accessible through the head dropdown in header
...
- **About** dialog has been refactored
  ...
  - editable to add or hide specific information if needed
...
- Error messages on create through a link pillbox
...
- The `version` parameter is not mandatory anymore for I/O module imports (and the `zip` parameter
  is automatically detected from file name extension when possible)
