...
- As already announced in the [breaking changes of version v6.2](v6-2#deprecated-features) the legacy **iText** libraries
  will be removed in next major version v7, there will be also an impact on the associated helpers `PDFTool` and `PDFDocumentExternalObject`
  helper classes that are still fully designed for iText in v6.
  If you use iText or the above helper classes in your code and plan to upgrade to v7 once it will be released
...
- `relologs`: same as 6.2 to merge children redologs in the parent list
