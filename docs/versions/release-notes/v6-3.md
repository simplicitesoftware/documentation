...
- Added `[ROOT]`, `[URL]` and `[ENV:<environment variable name>[:<default value>]]` substitutions in expressions
- Added the above `[ENV:]` substitutions in module settings
- Added the above `[ENV:]` substitutions in **loaded** values of system parameters
