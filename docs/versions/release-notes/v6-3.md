...
Docker images now enables a **default** health check command (`HEALTHCHECK`) calling the ping endpoint (`/ping`) every minute with a 3 seconds timeout after a start period of 5 minutes:

```text
HEALTHCHECK --interval=60s --timeout=3s --start-period=300s CMD curl --fail http://localhost:8080/ping || exit 1
```

If you have implemented a custom ping endpoint response, make sure this **default** health check command is compliant with it (or change it to comply with it).

> **Note**: the default values of the interval, timeout and start period should be fine for most cases but may be adapted
> in some **very particular** cases (e.g. where start time is over 5 minutes because it waits for a slow external system to start up).
