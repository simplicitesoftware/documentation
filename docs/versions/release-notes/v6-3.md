...
Git tagging of modules goes hand in hand with "[import specification based on Git tags](#git-tag-import)"
...
- if the Git tag is found the import is done on the tag
...
- if the Git tag is not found an error is raised
  (to import the Git repo HEAD it is thus now required to omit the `version` attribute).
