...
- The deprecated hooks `getData` and `getRenderFunction` without argument are now marked for removal and will be removed in next version
  if not already done, you need now to rename `getData` as `data` and add the `params` argument to `getRenderFunction`
