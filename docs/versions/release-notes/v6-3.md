...
- WCAG: added a table summary and sr-only on empty column, with new translated TEXT `TABLE_SUMMARY`, `ACTION_COL` and `ACTION_EMPTY`.
- WCAG: added `alt` with field value on `<img>` icon of ENUM field (without label on list).
