...
These changes may require action if you are concerned:

- Some development-oriented features/tools are now restricted to the [development mode](#development-mode)
  this has no impact on your projects, the only impact is that you now **should** start your development instances in development mode
  especially if you work in an external IDE and/or use Maven-level tasks such as linting or SonarQube code analysis.
- The legacy _FranceConnect_ authentication provider has been removed (it was already deprecated in previous minor versions)
  ...
  provider is also compliant with _FranceConnect+_ authentication service), this has no impact if you don't use the legacy
  _FranceConnect__ authentication provider
...
- The outdated legacy library `commons-discovery` has been removed (you can still add it as a custom shared lib if you use it, which is highly unlikely)
...
- The **GoogleMap** helper JS tool `gmap.js` is now deprecated and will be removed in next major version.
  if your custom code use it you should consider refactoring it to use the **Leaflet** helper JS tool which provides
  an abstraction layer on top of the map services providers.
  Note that the existing GoogleMaps inclusion methods will still include the GoogleMaps JS library
  but if your code is using the helper functions provided by `gmap.js` you should consider
  refactoring it to use directly the GoogleMaps JS API.
...
The compiler options can be customized if needed using the `JAVA_COMPILER_OPTIONS` system parameter (which defaults to `-g`) if you want,
for instance, to temporary ignore some types of warnings in your custom code (compilation warnings are non blocking but should rather be fixed than ignored).
...
An experimental feature allows to benefit from a Java Language Server (LSP) in the code editor.

At that stage it comes with some functional limitations
and with the constraint to use only spaces for your Java code indentation (no tabs).

It can be activated through the system parameter `FEATURE_FLAGS`:
...
:::warning
**Warning**: This feature requires that the instance's JVM is started in **development mode**.
This can be done by adding the `-Dserver.devmode=true` property to Tomcat JVM options
which can be set by using the `DEV_MODE` environment variable when using our Docker images
:::
...
- Added new `Tool.before/after` date comparison shorthand methods
- Added `MissingOverride` rule to the default Checkstyle ruleset in order to avoid signature mistakes when implementing a Java hook
