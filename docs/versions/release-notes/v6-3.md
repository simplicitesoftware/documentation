...
:::warning
**Warning**: This release note corresponds to a **future** minor version which is still in **ALPHA** stage and thus still in active development.
...
`FeatureFlag.IMAGES_IN_EXCEL_EXPORTS.isActive()`: `false` by default, true to allow export of images in excel
`FeatureFlag.TRUNCATE_TOO_LONG_TEXT.isActive()`: `true` by default to truncate on save too long text (except rich texts), `false` to return an error
...
and to override a flag per user session by code (`postLoadGrant`):

```java
if (getGrant().hasResponsibility("ONE_ADMIN_GROUP"))
  getGrant().getFeatureFlags().put("images_in_excel_exports", true);
```
