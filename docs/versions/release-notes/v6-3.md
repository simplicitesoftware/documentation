...
- if the Git tag is found the import is done on the tag (note that the cloned Git repository is then in **"detached HEAD" state**)
- if the Git tag is not found an error is raised (to import the Git repo HEAD it is thus now required to omit the `version` attribute).
