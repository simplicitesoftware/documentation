...
The legacy `ModuleGIT` external object has been removed.

Its features are now available on the `ModuleCommit` business object linked to the `Module` object.

This business object now displays the Git tag(s) of each commit.
