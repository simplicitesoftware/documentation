...
- The legacy _FranceConnect_ authentication provider has been removed (previously deprecated in earlier minor versions).
  Configuration as a generic OpenIDConnect authentication provider remains possible (the generic OpenIDConnect provider
  is also compatible with the _FranceConnect+_ authentication service). There is no impact unless the legacy _FranceConnect_
...
- The previous implementation of the generic OpenIdConnect authentication provider was using the access token for the `jwt_claims_mappings` mappings
  (which was a mistake because the access token can be a "dummy" token). It has been changed to use the ID token instead (which is the right approach).
  This change has no impact if ou don't use this type of mappings (e.g. if you use the user info mappings).
