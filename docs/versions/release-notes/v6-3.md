...
### New multi-reference search on list

When searching a reference, you can now select multiple references at once.

![](img/v6-3/multi_reference.gif)

### Custom context menu on lists
...
or **dynamic** using the dedicated hook `getMermaidChartSpec`, e.g.
...
public class AppTestMermaid extends com.simplicite.webapp.web.widgets.MyMermaidExtObject {
	...
	public String getMermaidChartSpec(Parameters params) {
		...
		// Else, defaults to static specification
