...
- The export objects' selection & ordering for a Dataset have been moved within the **Dataset**,
  and shall no longer be declared on the object's form with `exportorder`. [Dataset migration is documented here](make/project/datasets.md#using-62-or-earlier).
- OpenDataSoft has shut down it's legacy datasets API v1, our OpenDataSoft service object now uses by default the OpenDataSoft datasets API v2,
  OpenDataSoft service object(s) configured with an explicit version `1` it will be implicitly forced to `2`
  This configuration will result in a deprecation error in the logs. Projects that do not use OpenDataSoft datasets are not affected by this change.
...
### 6.3.3 (UNRELEASED) {#version-6.3.3}
