...
This release note corresponds to the **current** minor release of major version 6.

It is the **final** minor version of major version 6.

Major version 6 will soon be in its long term maintenance period.
...
- Some development-oriented features and tools are now limited to [development mode](#development-mode).
  This change does not impact projects directly, but development instances **should** now be started in development mode,
  particularly when working in external IDEs or when using Maven-level tasks such as linting, JaCoCo coverage measurement
  and/or SonarQube code analysis.
...
- OpenDataSoft has shut down it legacy datasets API v1, our OpenDataSoft service object now uses by default the OpenDataSoft datasets API v2,
  if you have OpenDataSoft service object(s) configured with an explicit version `1` it will be implicitly forced to `2`
  and you will get a deprecation error in the logs. If you don't use OpenDataSoft datasets you are not impacted with this change.
...
- See [the release note of minor version 6.0](/versions/release-notes/v6-0#compatibility-breaking-changes)
- See [the release note of minor version 6.1](/versions/release-notes/v6-1#compatibility-breaking-changes)
- See [the release note of minor version 6.2](/versions/release-notes/v6-2#compatibility-breaking-changes)

Deprecations
------------

The following deprecations may be removed in future releases and should be updated to anticipate their removal:
...
- The legacy `GOOGLE_FONT` system parameter is now deprecated in favor of the fonts defined at the theme level.
...
- The legacy ChartJS version 2 will be removed in next major version, any custom code using this legacy version should be refactored
  to use either the current default version 3 (or next version 4, that will become the default version
  in next major version, keeping in mind that the standard charts are not compliant with this next version 4)

### Deprecated iText

As a reminder, the legacy **iText** library will be removed in next major version v7 and there will be also breaking change impacts
...
Project code still using the legacy iText lib or the related helper classes should be refactored to use the **PDFBox** libraries before upgrading to v7,
as iText support will be removed (this code will not compile anymore in v7).

### Deprecated Google Map

UI-level Google Maps features are now deprecated and will be removed in next major version v7.
...
obj.search(list => { /* do something with list */ }, filters);
// Should be refactored (since v6):
//  - to use the promise syntax
obj.search(filters).then(list => { /* do something with list */ });
//  - to use the await operator (if in an async function)
...
The OpenStack Swift protocol is not supported anymore, the related methods in the `CloudStorageTool` helper
class are now deprecated and will be removed in v7 along with the corresponding Apache jClouds lib
(Maven ID: `org.apache.jclouds.api:openstack-swift`).
...
- See [the release note of minor version 6.0](/versions/release-notes/v6-0#compatibility-breaking-changes)
- See [the release note of minor version 6.1](/versions/release-notes/v6-1#deprecations)
- See [the release note of minor version 6.2](/versions/release-notes/v6-2#deprecations)
...
- the map renderer has been migrated to the [Leaflet](https://leafletjs.com) library (instead of Google Maps)
  for coordinate fields and place maps
...
- Re-numbering through bulk action the current search with an increment value
...
The search configuration is applied to the `main`, `home` and `panel` instances.
For `ref` and `datamap` instances, the search configuration is as follows :

- Column search : `visible`
- Dialog search : `hidden`
- Docked search : `hidden`
- Predefined search : `hidden`

For all other instances, the search configuration is as follows :

- Column search : `collapsed`
- Dialog search : `enabled`
- Docked search : `hidden`
- Predefined search : `hidden`
...
- by default a screenflow is undo-able, but a long process not
...
Git tagging of modules goes hand in hand with "[import specification based on Git tags](/versions/release-notes/v7-0#git-tag-importspec)"
...
Details are provided in the [Code Editor documentation](docs/docs/codeeditor.md)
...
This feature is still **experimental** thus not yet enabled by default [see below](/versions/release-notes/v6-3#new-featureflags) to enable it.
...
For example, flag names are in lower case here:
...
New servlet `/ui/sse` (asynchronous) to push messages from server to clients through https
...
- `[DATE<:offset>]`
- `[DATETIME<:offset>]`
- `offset` = a signed number with unit (`y` years, `m` months, `w` weeks, `dw` days without week-end,
  `d` days, `h` hours, `mi` minutes, `s`seconds, `ms` milliseconds)
...
In development environments, the development mode **must be enabled** explicitly.

The development mode is set by the `server.devmode=true|false` JVM argument (or the `DEV_MODE` environment variable)

**Not being** in development mode prevents the use of the following features:

- the `devinfo` services (e.g. used by legacy VSCode plugin)
- the `/maven` embedded Maven repository endpoint
- The database & document database access pages
- the legacy _Log viewer_ page

:::tip[Reminder]

Java code formatting (which requires some additional JVM capabilities to be enabled)

:::

Note also that the health check is forced _minimal_ when **not in** development mode.

As a consequence of the extension of the development mode, the legacy security level has been deprecated
...
with a call interval of 1 minute and a 3 seconds timeout (after a start period of 10 minutes during which
...
The new `BusinessObject` class works like a try-with-resources builder to chain classic data access methods
...
```java
// isolated instance of User object with full rights
try (BusinessObject bo = getGrant().getBusinessObject("User")) {
	...
	.search((index, count, obj) -> {
		try {
			...
			.withValues(Map.of(
				...
				"usr_lang_pref", "FRA"))
			.validateAndSave() 
			// optional select values from DB
	...
	// - Previous user rights are restored on the object
}
...
try (BusinessObject bo = new BusinessObject(getGrant(), "MyObject")) {
	bo
	.getTmpObject() // use the tmp instance, or specific one with getInstance("MyInstance")
	.withAllAccess(); // same as withCRUD(true, true, true, true)
	.preserveContext() // save the instance definition to be restored on close
	.forCreateOrUpdate(Map.of( // upsert on key fields
		"myKeyField1", "bob",
		"myKeyField2", "123"))
	.withValue("myField1", "value1")
	.withValue("myField2", "value2")
	.validateAndSave(msg -> AppLog.info("save = "+msg)) // exec all in a single synchronized block
	.returns(msg -> AppLog.info("msg = "+msg)) // optional, to get the return message of the save
	.action("MyActionName", // invoke an action on the object
		Map.of("param1", "value1"), // with simple parameters
		msg -> AppLog.info("action result = "+msg)); // to get the return message of the action
}
	...
	// - preserveContext is not relevant for isolated instance because it is destroyed
...
By default the tracker get its action label.
Now the tracker is allowed to change the dialog title during the job processing:
...
Pre-release backports
---------------------

- Backported [from v7.0](/versions/release-notes/v7-0#git-tag-importspec) git tag import specification
- Backported [from v7-0](/versions/release-notes/v7-0#optional-importspec-version) optional import specification module version

Post-release
------------

### 6.3.0-RC1 (2025-12-05) - release candidate 1 {#version-6.3.0-beta-RC1}

- Initial release candidate for minor version 6.3. See above for details.

### 6.3.0-RC2 (2025-12-19) - release candidate 2 {#version-6.3.0-beta-RC2}

- Backported [from v7.0](/versions/release-notes/v7-0#external-objects-http-headers) the possibility
  to add custom HTTP headers in the response of the external objects.
- Fixed missing `com.simplicite.util.engine.PlatformHooksInterface` in public classes
  and removed legacy backward-compatibility class `com.simplicite.util.PlatformHooksInterface`.
- Fixed metadata forced reloading for UI instances with a parent object to apply rules based on parent context/values
  (not just previous reference select instance, but also panel, pillbox, datamap...).
- Fixed duplicate objects in fulltext search.
- Backported [from v7.0](/versions/release-notes/v7-0#json-schema-tools-improvements) improvements on the `JSONSchemaTool` JSON schema helper class
  and updated JSON schema lib
- Changed error to warning when sometimes `CRON_LOCK` is re-assigned in the meantime on a cluster (Kubernetes)
- Fixed create button on list activity to use the hook `isCreateEnable`
- Fixed environment variables an/or/ JVM properties substitutions in module's settings
- Fixed dataset objects validation rules

### 6.3.0-RC3 (2026-01-06) - release candidate 3 {#version-6.3.0-beta-RC3}

- Upgraded vulnerable Java and JavaScript libs
- Improved Java Language Server experimental features
- Fixed warnings on legacy 3-letters time zone codes
- Fixed select all on the graphical modeler
- Fixed environment variables overriding of `USE_<IO|API>[_TESTER]` system parameters
- Improved accessibility WCAG: region roles, aria-label, aria-labelledby and missing titles
- Replaced isolated headers `<h3>`/`<h4>` on list/form/search with simple styled `<div>` to respect WCAG rules
- Fixed import module progress bar

### 6.3.0-RC4 (2026-01-07) - release candidate 4 {#version-6.3.0-beta-RC4}

- Downgraded Rhino libs that were causing issues on constraints configuration
- Minor fixes on the theme editor

### 6.3.0-RC5 (2026-01-10) - release candidate 5 {#version-6.3.0-beta-RC5}

- Upgraded Java and JavaScript libs
- Fixed "Readonly form" contextual helps not displaying
- Added parameter `USE_SSE` to disable the server-side-event services when not available/usable
- Added the stylistic stylelint plugin and associated rules to lint the module's custom CSS/LESS resources
  (this is needed because stylistic rules have been removed from the latest versions of the stylelint lib)

### 6.3.0 (2026-01-15) {#version-6.3.0}

- Initial release for minor version 6.3.
- Fixed inlined forms via a virtual link not using the SQL filter.
- Fixed search errors not displaying.
- Apply settings of list of values `FORMAT.NUM` on the big-decimal field.

### 6.3.1 (2026-01-23) {#version-6.3.1}

- Fixed do not `followLink` on HTML/rich text.
- Fixed an issue where inlined objects displayed as tabs showing default templates.
- Fixed mapped API to return "raw" JSON for long text fields with JSON rendering.
- Backported [from v7.0](/versions/release-notes/v7-0#custom-build-artifacts) possibility to customize the modules' build artifacts (`pom.xml` and `gradle.*`).

### 6.3.2 (2026-01-30) {#version-6.3.2}

- Upgraded Java libs.
- Fixed counter widgets layout issues.
- Fixed `ADMIN_SYSTEM` not unconditionally forced anymore for `designer` (only for `system`),
  it now takes into account the `ADMIN_SYSTEM` user parameter value (which is set to `yes` out of the box).
- Fixed missing usage of `base_url` of OAuth2/OpenIDConnect providers in `AUTH_PROVIDERS` to build other URLs if non absolute.
- Backported [from v7.0](/versions/release-notes/v7-0#well-known-endpoints) OpenIDConnect/OAuth2 "well known" metadata endpoints.
- Backported [from v7.0](/versions/release-notes/v7-0#custom-build-artifacts) the preservation of the module's `README.md` across Git commits.
- Backported [from v7.0](/versions/release-notes/v7-0#rest-mapped-apis-permalinks) permalink support for the REST mapped APIs.

### 6.3.3 (2026-02-06) {#version-6.3.3}

- Fixed datalink synchronization of notepad field with textual rendering.
- Fixed `Cache-control` HTTP header value for better control on cache management.
- Fixed collapsed panel accessibility on Tab/Enter/Arrow keys.
- Backported [from v7.0](/versions/release-notes/v7-0#dedicated-code-editor-object-instances) dedicated object instances for the UI code editor.
- Added helper method `MarkdownTool.fromHTML` for HTML to Markdown conversion (based on an additional Java lib).
- The `ResponsiveExternalObject` hooks `getData` and `getRenderFunction` without argument are now marked for removal and will be removed in next version.
  the [data](https://platform.simplicite.io/6.3/javadoc/com/simplicite/webapp/web/ResponsiveExternalObject.html#data(com.simplicite.util.tools.Parameters))
  and [getRenderFunction](https://platform.simplicite.io/6.3/javadoc/com/simplicite/webapp/web/ResponsiveExternalObject.html#getRenderFunction(com.simplicite.util.tools.Parameters))
  must be respectively used instead.
- Fixed header logo accessibility with new text `BACK_HOME` and application title on `img alt`.

### 6.3.4 (2026-02-16) {#version-6.3.4}

- RGAA: added a table summary and sr-only on empty column, with new translated TEXT `TABLE_SUMMARY`, `ACTION_COL` and `ACTION_EMPTY`.
- RGAA: added `alt` with field value on `<img>` icon of ENUM field (without label on list).
- Link rendered as `Pillbox` now hides the input and create button when the max occurrences is reached.
- Backported [from v7.0](/versions/release-notes/v7-0#module-init-io-service) the new `moduleinit` I/O service.
- Fixed empty context URL for I/O grant.
- Added `--legacy-peer-deps` in the `npm install` called during the `validate` phase of modules' `pom.xml`.
  This avoids potential conflict errors when installing the `"latest"` versions of JavaScript development dependencies
  optionally referenced in the modules' `package.json` (ESLint, StyleLint, ...).
- Backported [from v7.0](/versions/release-notes/v7-0#custom-build-artifacts) changed module's `README.md` content generation.
- Fixed cascade copy of multi-documents field.
- Fixed inlined 0,1/1,1 objects cascade copy and parent data reloading.
- Fixed empty form from home instance of a specific "select" object.

### 6.3.5 (2026-02-20) {#version-6.3.5}

- Backported [from v7.0](/versions/release-notes/v7-0#improved-cloudstoragetool) support for user metadata in cloud storages.
- Fixed JSON body parsing to take into account explicit `null`s.
- Fixed incorrect loading of platform hooks at startup in some cases.
  (note: the workaround to this issue was to force a global clear cache after the platform startup).
- Backported [from v7.0](/versions/release-notes/v7-0#outdated-apache-commons) removal of the `xml-apis` lib (conflicting w/ JVM module).
- Fixed missing `com.simplicite.util.engine.DispositionInterface` in public classes.
- Added Skiplinks

### 6.3.6 (2026-03-12) {#version-6.3.6}

- Backported [from v7.0](/versions/release-notes/v7-0#git-commit-list-ordering) default "topological" ordering of module's Git commits.
- Backported [from v7.0](/versions/release-notes/v7-0#additional-libs-in-the-light-variant)
  inclusion of the _Docx4j_, _JClouds_ and _Tika_ libs in the "light" variant.
  And the `/maven` endpoint is now only exposing the dependencies for the considered variant.
- Fixed guide recorder generating undefined selectors.
- Fixed potential _not found_ on API tester page depending on the configuration of the `URI_MAPPINGS` system parameter.
- Added prefix in the objects' metadata.
- Backported [from v7.0](/versions/release-notes/v7-0#extended-footer) optional footer drop-up panel named `FOOTER_ADDON`.
- Backported [from v7.0](/versions/release-notes/v7-0#uniformized-borrow-and-return-object) `Grant.borrowObject`/`Grant.returnObject` methods
- Fixed visible textarea of ​​an HTML field before tinyMCE parses its content.
- Fixed missing Ace editor custom components for Simplicité expressions.
- Fixed order on `select` objects' enum fields to be by code by default.
- Fixed missing check of the mail service to enable forgot password and/or register feature.
- Added `ACCESS_DENIED` exception in `BusinessObject` tool to prevent missing rights on select/search/stream.
- Fixed hidden fields on list by a front constraint.
- Fixed empty boolean fields being set to false after a front constraint.
- Fixed missing alteration of existing object table for permalink.
- Improved RGAA accessibility

### 6.3.7 (2026-04-03) {#version-6.3.7}

- Upgraded Java and JS libs.
- Fixed front read-only constraint on document/image.
- Backported [from v7.0](/versions/release-notes/v7-0#metadata-hooks) `preMetadata` and `postMetadata` hooks.
...
- Backported [from v7.0](/versions/release-notes/v7-0#module-audit-io-service) improvements on modules audit.
- Bumped OpenAPI schema version to 3.0.4 (previously 3.0.2).
- Fixed `Grant.rollbackTransaction`.
- Improved RGAA accessibility

### 6.3.8 (2026-04-24) {#version-6.3.8}

- Upgraded Java libs.
- Fixed additional search-spec of view item within a `select` object.
- Fixed regression on dependencies exclusions when building Maven repository.
- Added Java accessors to the rule impacts of constraints.
- Helper CSS classes `.area-first-col` and `.area-last-col` on list to find the first/last visible columns of areas.
- Added log traces for object indexation.
- Backported [from v7.0](/versions/release-notes/v7-0#single-object-re-indexation) the single object re-indexation action.
- Backported [from v7.0](/versions/release-notes/v7-0#shared-code-publication-methods) the possibility to configure
  a publication method implemented in a shared code.
- Fixed `ObjectDB.checkObjectPropConstraints` to check only impacted object property (do not apply rules on field/target).
- Fixed inappropriate warnings within forgot password feature.
- Fixed constraint impacted-field detection when field names are substrings of other field references.
- Backported [from v7.0](/versions/release-notes/v7-0#new-supervision-features) new supervision JMX operations.
- Improved RGAA accessibility

### 6.3.9 (2026-05-08) {#version-6.3.9}

- Fixed _Register_ form and optimized form if only one profile is registerable.
- Fixed outstanding _I forgot my password_ button after submitting form.
- Fixed anonymize objects containing physical columns of the user to anonymize.
- Added `ObjectDB.getConstraint()` accessor.
- Fixed feature "server-side event" against the `USE_SSE` value.
- Fixed legacy Rhino script presence check for shared scripts.
- Fixed swallowing `/r` tiny resource URI pattern without `?`.
- Fixed forced Git tag action parameter.
- Added a contextual help button on N,N pillbox for context "linked list".
- Added a `PlatformHooks.getEndPointURL()` to override the default URL based on the hostname,
  for specific inter-nodes communication in a cluster (e.g. Kubernetes).
- Fixed save of predefined search.
- Added auto-close mechanism for undestroyed Grant,
  and a JMX counter of Grant instances, with log events to show remaining Grant instances in memory.
- Fixed I/O always allowed for platform nodes intercommunication.
- Fixed useless synchronize when calling custom API external objects on the API (`/api/ext`) endpoint.
- Fixed random value on form editor for new _Object_ type fields.
- Added a bulk action to rebuild permalinks of all records of an object.
- Fixed `$.jqPlot` text rendering on Chrome.

### 6.3.10 (2026-05-28) {#version-6.3.10}

- Upgraded Java libs.
- Fixed manually triggered specific channel notification.
- Fixed warnings about legacy JS syntax in some modeler scripts.
- Added `statusField` setting for cards widget.
- Fixed missing notification unsubscribe grant for `NOTI_READER` group.
- Robustness of populate of nullable key to detect potential non-`null` empty values (e.g. created by external import processes).
- Fixed code editor height.
- Fixed default name of generated JS resource for an external object (`CLASS` instead of legacy `SCRIPT`).
- Added `HTMLTool` methods variants to get HTML/Markdown resources contents with URLs substitutions.
- Added remote client IP address field to the session history object.
- Fixed States navbar to hide missing and non-visited status before the last visited one.
- Fixed access to search by columns when no-search is set on object.
- Fixed permissions on model template selector.

### 6.3.11 (2026-06-19) {#version-6.3.11}

- Added a front hook `onReadForm` to be separated from `beforeSave` logic.
- Fixed access to object trays and metrics even if all states are hidden in menu.
- Backported [from v7.0](/versions/release-notes/v7-0#preferred-format-settings-for-module) preferred format settings for module.
- Backported [from v7.0](/versions/release-notes/v7-0#legacy-rhino-scripts-check) startup blocking error if legacy Rhino scripts are still present.
- Fixed email regex to allow legit stacked dashes/plus signs.
- Backported [from v7.0](/versions/release-notes/v7-0#unsupported-database-management)
  fallback to SQL99 for unknown (= unsupported) additional database types.
- Fixed custom action auto-save losing object data when retrying in an error state.

### 6.3.12 (2026-07-10) {#version-6.3.12}

- Improved UI robustness to detect infinite loop in a reflexive tree (e.g. the object fields panel rendered as a tree)
- Fixed language inconsistency during the _forgot password_ procedure
- Added deprecation warning if still using the outdated `OAUTH2_PROVIDER` system parameter instead of the `AUTH_PROVIDERS` system parameter
  (this is a final warning as the support for this legacy system parameter will be removed in next major version).
- Added system parameter `PANEL_FIXED_FILTER = yes` with the same behavior as `PANEL_FILTER` but for fixed filters.
- Added boolean `showFixedFilters` to `$ui.displayList` options, default true if `PANEL_FIXED_FILTER=yes`.
  `showFilters` is still available for non-fixed filters.
- Fixed search configuration for `home` and `panel` instances.
- Backported [from v7.0](/versions/release-notes/v7-0#configurable-openidconnect-logout-mode) configurable mode
  for the logout endpoint of a OpenIDConnect/OAuth2 provider.
- Optimized huge list rendering without pagination.
- Deprecated `BusinessObjectTool.select[ForXxx]` methods.
- Backported [from v7.0](/versions/release-notes/v7-0#password-platform-hooks) the new `generatePassword` and `hashPassword` platform hooks.
- Backported [from v7.0](/versions/release-notes/v7-0#active-sessions-check) the new active sessions check scheduled action,
  note that it is disabled by default, if you need it you must explicitly enable it.
- Fixed document fields missing from change log.

### 6.3.13 (2026-07-24) {#version-6.3.13}

- Upgraded Java libs.
- Fixed JSON module diff for translations.
- Fixed `ServletTool.setObjectField` to parse meta-object as JSON object `{ object, item }`.
- Fixed permission on object field inheritance.
- Fixed missing branch in the `module-info.json` file.

### 6.3.14 (2026-07-31) {#version-6.3.14}

- Upgraded Java libs.
- Fixed follow link of rendered enum on list.
- Fixed missing links on lists for enum fields.
- Improved logging, in particular for authentication events `xAUTHSCxxxx`
- Removed legacy module auto update cron task, action and flag.
- Fixed and improved ZIP/JAR browser.

### 6.3.15 (2026-08-28) {#version-6.3.15}

- Upgraded Java and Javascript libs.
- Removed useless `LSP_CONFIG` system parameter.
- Improved robustness vs the potential unavailability of the ECJ Java compiler library.
- Fixed error messages display on time sheets.
- Simplified active sessions check action: removed support for a potential refresh token duration.

### 6.3.16 (2026-09-09) {#version-6.3.16}

- Upgraded Java libs.
- Security fix.
- Fixed cascade copy of pillbox and 1-N links.
- Fixed duplicate error messages for hidden fields.
- Removed the object's sub-menu when it contains only the "Show all" option.
- Fixed enumeration filters removed from search inputs.
- Fixed check active sessions token duration vs cron task frequency.

### 6.3.17 (2026-09-17) {#version-6.3.17}

- Upgraded Java and Javascript libs.
- Backported [from v7.0](/versions/release-notes/v7-0#help-from-json-schemas) JSON schema-derived help for widgets external objects and system parameters.
- Backported [from v7.0](/versions/release-notes/v7-0#accordion-widget) accordion widget.
- Backported [from v7.0](/versions/release-notes/v7-0#time-line-widget) time line widget.
- Fixed regression on TOTP authentication.

### 6.3.18 (UNRELEASED) {#version-6.3.18}

- Upgraded Java and Javascript libs.
- Improved UI link panel performance through lazy-loading.
- Fixed field search on inlined objects.
- Fixed extended fields not mapped in the form template (e.g. on "more" click, field search...).
- Backported [from v7.0](/versions/release-notes/v7-0#default-public-rights) "historical" public rights removal for security reasons.
- Fixed issue in template editor for image fields with image only rendering
- Fixed missing format parameters for `commitmodule` I/O service
- Backported [from v7.0](/versions/release-notes/v7-0#user-level-schema-for-standard-rest-apis) `objects=true` parameter
  for the root REST API (`/api/rest`) to get the list of accessible business and external objects.
