...
### 6.3.13 (UNRELEASED) {#version-6.3.13}
...
- Backported [from v7.0](v7-0#module-info-platform-version) change on the `module-info.json` file:
  now the `platform` attribute indicates the _minor version_ instead of the _revision_.
  The _revision_ being indicated in the new `platform_revision` attribute.
