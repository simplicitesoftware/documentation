...
When the import specification of a module uses a Git repository the `version` attribute (if set) is now used to match a tag:

- if a matching tag is found the import is done on the tag (note that the clone Git repository is then in "detached HEAD" state)
- if no matching tag is not found the behavior is the usual behavior
