...
- Removed wrapper for Rhino hooks implementation:
  ...
  - Classes `ScriptedObjectDB`, `ScriptedProcessus`, `ScriptedDisposition`, `ScriptedExternalObject` and `ScriptedPlatformHooks` has been removed
...
Please note that additional compatibility breaking changes were introduced in the previous minor version:
...
Please note that additional compatibility deprecations were introduced in the previous minor version:
...
- Added PostgreSQL `websearch_to_tsquery` as default parser for fulltext search (can be inhibited with param `PGSQL_TSQUERY=to_tsquery`)
...
- Security fix: updated Lucene lib (see [this CVE warning](https://nvd.nist.gov/vuln/detail/CVE-2024-45772))
...
- Backported unit tests I/O service improvement allowing the use of the import specification syntax
