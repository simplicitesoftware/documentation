...
If you have external objects of type `ResponsiveExternalObject` implementing the `getRenderFunction` hook
to generate a render JS logic from the server-side you **must** to remove the enclosing `[async] function() { ... }` around the JS code.
