...
The `version` property in the import specification is now optional, **if none is specified the import is done in all cases**.
...
- if found the Git repository is checked out on this tag
- if not found the repository is checked out on the head

In all cases the import is then done on the checked out state of the module
