...
This old verison of this HTML editor is not maintained anymore and the latest version 8 is only for commercial usage.
The UI will now use the open-source Quill (see below).

Features
--------

- New "Copy to Clipboard" setting for fields, setting it to *Yes* will show a clipboard icon on the field's addons, clicking it will copy the displayed value. The value is *No* by default.
- New `SocialPostMention` Notification : A mentioned user (@user) will receive a notification when a post is created.
...
For customized images at the Tomcat level, note that version 7.x images use Tomcat 11, which may have slight differences
...
### Import spec improvements

The `version` property in now optional, if none is specified the import is done in all cases.

If the module spec is a Git repository and the `version` property is specified, the import will clone the
Git repository and will check for a Git tag matching the specified version:

- if found the Git repository is check outed on this tag
- if not found the repository is check outed on the head

In all cases the import is then done on the check outed state of the module
