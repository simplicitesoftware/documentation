...
### Improved Import Spec

The `version` property in the Importspec file is now optional, if none is specified the import is done in all cases.
...
- if found the Git repository is checkouted on this tag
- if not found the repository is checkouted on the head
