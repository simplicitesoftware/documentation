...
- if the UI code uses thoses parameters, it must be changed to use `obj.context` instead of `obj.locals`
...
### Configurable OpenIdConnect logout URL call method

For a OpenIDConnect/OAuth2 provider it is now possible to specify the call method of the logout endpoint
using the `logout_url_method` setting in the `AUTH_PROVIDERS` system parameter.

The possible values are:

- `GET` = client side redirect to the logout URL
- `POST` = server side call to the logout URL and client side redirect to the post-logout URL
