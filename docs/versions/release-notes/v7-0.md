...
### ObjectDB extends BusinessObjectTool

The `ObjectDB` class now extends the `BusinessObjectTool` class to call helpers directly.

- `obj.getTool()` is still available but deprecated, the new preferred way is to use the `ObjectDB` instance directly.
- e.g. use `obj.validateAnsSave()` instead of `obj.getTool().validateAndSave()`.
- some incompatible signatures must be easily refactored as follows:

| Previous call                  | New method with Exceptions | Remarks                                                  |
|--------------------------------|----------------------------|----------------------------------------------------------|
| `getTool().validate()`         | `validateWithException()`  | `validate()` is already defined to return `List<String>` |
| `getTool().save()`             | `save(true, false)`        | `save()` is already defined to return `String`           |
| `getTool().save(timestamp)`    | `save(timestamp, false)`   |                                                          |
| `getTool().create()`           | `create(false)`            | `create()` is already defined to return `String`         |
| `getTool().update()`           | `update(true, false)`      | `update()` is already defined to return `String`         |
| `getTool().update(timestamp)`  | `update(timestamp, false)` |                                                          |
| `getTool().del()`              | `delete(false)`            | `del` is already defined to return `String`              |
| `getTool().delete()`           | `delete(false)`            | `delete()` is already defined to return `String`         |

All other tool methods are still available and unchanged (`validateAndSave`, `getForCreate`...).
