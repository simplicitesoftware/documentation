...
All the internal contextual (cached data) have been isolated into `context` to keep the `locals` safe and dedicated to application code and front hooks.

- if the application code uses thoses parameters, it must be changed to use `obj.context` instead of `obj.locals`
- the new internal context can evolve to become breaking changes in future versions. So it is not recommended to use it in the application code:
