...
### Improved configuration audit

The audit social posts are now attached to the `system` user (vs `designer` previously).

A module audit can also be now invoked thru the I/O endpoint using the dedicated `moduleaudit` service which requires the module name `module` parameter.
