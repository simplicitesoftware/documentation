...
##### Changed rendering of UI components external objects

The `render` hook of UI components external objects JS class extending `Simplicite.UI.ExternalObject`
were missing a container argument it as been added and, so as to comply with other JS hooks conventions,
it has been positioned as the first argument.

Therefor you **must** refactor your external objects' JS class code to add this argument:

```javascript
// the previous version signature was async render(params, data), it must be changed to:
...
If you don't do the above refactoring, your external object won't render properly

:::

Moreover, if you have external objects of type `ResponsiveExternalObject` implementing the `getRenderFunction` hook
to generate the render JS code from the serveur side you **must** to remove the enclosing `[async] function() { ... }` around the JS code.

Now the `getRenderFunction` **must** only contain the **body** of the rendering function. This is due to the way this JS code
is now evaluated within a `new AsyncFunction()` rather than using the `eval`.
