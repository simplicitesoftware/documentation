...
|--------------------------------|----------------------------|---------------------------------------------------------|
| `getTool().validate()`         | `validateWithException()`  | `validate()` is already define to return `List<String>` |
| `getTool().save()`             | `save(true, false)`        | `save()` is already define to return `String`           |
| `getTool().save(timestamp)`    | `save(timestamp, false)`   |                                                         |
| `getTool().create()`           | `create(false)`            | `create()` is already define to return `String`         |
| `getTool().update()`           | `update(true, false)`      | `update()` is already define to return `String`         |
| `getTool().update(timestamp)`  | `update(timestamp, false)` |                                                         |
| `getTool().del()`              | `delete(false)`            | `del` is already define to return `String`              |
| `getTool().delete()`           | `delete(false)`            | `delete()` is already define to return `String`         |

All other tools methods are still available and unchanged.
