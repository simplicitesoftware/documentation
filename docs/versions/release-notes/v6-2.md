...
- Backported [from v6.3](v6-3/#compatibility-breaking-changes) change on the `jwt_claims_mappings` mappings of the generic
  ...
  This change is needed to allow the access token to be a "dummy" token in some cases. It has no impact if you are using
  another type of mappings (e.g. the usual `userinfo_mappings`)
