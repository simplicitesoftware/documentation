...
This minor version is the **last** version 5 minor version of major version 5.
...
- FullCalendar version 3 is now **deprecated** and will be removed in the next minor release (you can still use version 3 but you now **must**
  consider migrating your custom code to version 5 or at least version 4, see below)
...
To upgrade your specific code see [this document for v3](https://fullcalendar.io/upgrading-from-v3) or [this document for v4](https://fullcalendar.io/upgrading-from-v4)
...
This `lax` value the appropriate value when using external OpenIDConnect/SAML authentication.
If you don't use such external authentication you can set it to `strict`.

You can revert to Tomcat's default by setting it to `unset` in the `CookieProcessor`
block of the `META-INF/context.xml` descriptor (or by commenting this block).

### EasyModeAdapter deprecated

Old adapters to import legacy settings through `quick param` (V3 tooling) has been removed, and related code is deprecated.
...
- The system parameter `USE_SOCIAL` has been removed, but still valued to true at logon
  if user has responsibility `SOCIAL_USER` or `SOCIAL_ADMIN` (for backward compatibility)
...
- New `DataLink` configuration to synchronize data between several applications
  - Set a list of named hosts with credentials to be accessed through REST APIs
...
- New type of `Action` to launch long running jobs started through UI in a `isolated session`
...
- New auto-incremental field
  - _Auto-incremental syntax_ configuration field on Field object allows to automatically increment field value according to a
    specified syntax when creating a new record.
...
- New **Notification** feature
  ...
  - Users can subscribe / unsubscribe from their Notifications (requires the Notification to be non-mandatory and user is a Recipient of the Notification)
    - Subscriptions are based on the Notification / Channel couple - meaning that for a given Notification broadcasted
      over Mail and Internal channels, a user can unsubscribe from the Mail channel.
  - _Channels_ :
...
- Ping platform nodes to clean `m_pf_node` periodically through the cron HealthCheck

- Import License key file through UI action
...
- Added [Bootstrap icons 1.5.0](https://icons.getbootstrap.com)
...
- Improved log push through websocket to users with `USE_WEBSOCKET_LOG`
...
- Fixed effective date in case of data import through adapter
...
- Backported menu accessibility through `ALT+M` + arrow-keys + `ENTER`
...
- Backported UI form's HTML text input type are now set to `number`, `tel`, `email` or `url` instead of plain `text`
...
- Fixed redrawing an empty document field through a constraint of visibility
...
- Backported [from v6.3](v6-3#mail-service-configuration) support for `MAIL_SERVICE` overriding by the
  same name environment variable (or `mail.service` JVM property) and backported support for YAML syntax instead of JSON
...
- Backported [from v6.3](v6-3#environment-variable-based-configurability) `[ENV:<environment variable name>]`
  substitutions in module settings and system parameters
...
- Backported [from v6.2](v6-2#version-6.2.10) mention of the the `origin` module remote Git URI (if defined)
  to avoid ambiguities vs the instance's module Git URI
...
- Backported [from v6.2](v6-2#version-6.2.11) module filtering support on groups displayed
  in the _Authorization_ step of the _Create Activity_ system business process
...
- Backported [from v6.2](v6-2#version-6.2.13) improvements on platform's "garbage collection" tasks
  to avoid dropping auth-only sessions as "unknown" sessions

### 5.3.75 (2025-07-31) - maintenance revision {#version-5.3.75}

- Backported [from v6.2](v6-2#version-6.2.14) revert on JSON rendering on some system parameters using JSON syntax
  to avoid unordered "random" JSON reformatting
...
- Backported [from v6.2](v6-2#version-6.2.14) set HTTP status and headers by default in the response
  before calling the `customErrorResponse` platform hook
...
### 5.3.78 (2025-09-30) - maintenance revision {#version-5.3.78}

- Backported [from v6.2](v6-2#version-6.2.17) fix `populateForeignKey` in case of multi-reflexive links
- Backported [from v6.2](v6-2#version-6.2.17) fix for list's column orders when no areas are displayed
- Backported [from v6.2](v6-2#version-6.2.17) fix for issue where the list ordering preference was shown even when the object's
  configuration wasn't relevant (i.e. no field areas visible on the list)
- Backported [from v6.2](v6-2#version-6.2.17) fix on missing standard packages/classes in the public Java API (e.g. the annotations package)
- Backported [from v6.3](v6-3#new-helper-methods) `Tool.before/after` date comparison shorthand methods

### 5.3.79 (2025-10-02) - maintenance revision {#version-5.3.79}

- Filtered `com.simplicite.util.annotations` package from the standard imports of Rhino scripts
  to avoid class name ambiguity errors (note that adding this package in previous revision was required
  to allow Java class compilation outside of Simplicité, e.g. in an external IDE, for code using these annotations).

### 5.3.80 (2025-11-14) - maintenance revision {#version-5.3.80}

- Backported [from v6.2](v6-2#version-6.2.18) fix on XML import final `status KO` in case of unknown references after recycling
- Backported [from v6.3](v6-3#new-helper-methods) new `Tool.lower/upper/capitalize` null-safe string case manipulation methods
- Backported [from v6.2](v6-2#version-6.2.19) fix on `/io` page in case of non root deployment
- Updated vulnerable JFreeChart Java lib and BootBox and Vue additional JS libs

### 5.3.81 (2025-12-05) - maintenance revision {#version-5.3.81}

- Security fixes
- Backported [from v6.2](v6-2#version-6.2.19) reading of the protected `PATCH_LEVEL` system parameter by the ping endpoint
  (instead of the `VERSION` that may be deleted), this avoids potential falsely negative ping responses.
...
- Updated  vulnerable JGit Java libs and XTerm additional JS lib
- Backported [from v6.2](v6-2#version-6.2.19) security fixes
- Backported [from v6.2](v6-2#version-6.2.19) missing `initUpdate` on state transition from a row action on list
- Backported [from v6.2](v6-2#version-6.2.19) fix for duplicates of auto-incremented fields

### 5.3.82 (2026-01-19) - maintenance revision {#version-5.3.82}

- Upgraded Java libs
- Backported [from v6.3](v6-3#version-6.3.0-beta-RC2) fix on environment variables / JVM properties substitutions in module's settings
- Backported [from v6.3](v6-3#version-6.3.0) Apply settings of list of values `FORMAT.NUM` on the big-decimal field

### 5.3.83 (2026-02-06) - maintenance revision {#version-5.3.83}
...
- Backported [from v6.3](v6-3#version-6.3.3) fix on `Cache-control` HTTP header value for better control on cache management

### 5.3.84 (2026-02-11) - maintenance revision {#version-5.3.84}
...
- Fixed missing JClouds indirect dependencies
...
### 5.3.86 (2026-04-03) - maintenance revision {#version-5.3.86}
...
- Backported [from v6.3](v6-3#version-6.3.7) fix `Grant.rollbackTransaction()`

### 5.3.87 (2026-04-24) - maintenance revision {#version-5.3.87}

- Upgraded Java libs.
- Backported [from v6.3](v6-3#version-6.3.8) new log traces for object indexation.
- Backported [from v6.3](v6-3#version-6.3.8) fix on inappropriate warnings within forgot password feature.
- Backported [from v7.0](v7-0#new-supervision-features) new supervision JMX operations.

### 5.3.88 (2026-05-08) - maintenance revision {#version-5.3.88}

- Backported [from v6.3](v6-3#version-6.3.9) fix on _Register_ form.
- Backported [from v6.3](v6-3#version-6.3.9) fix on outstanding _I forgot my password_ button after submitting form.
- Backported [from v6.3](v6-3#version-6.3.9) fix on swallowing `/r` tiny resource URI pattern without `?`.
- Backported [from legacy v6.2](v6-3#) the `customPingResponse` platform hook to customize the `/ping` JSON response if needed.
- Backported [from v6.3](v6-3#version-6.3.9) fix on useless synchronize when calling custom API external objects on the API (`/api/ext`) endpoint.

### 5.3.89 (2026-05-28) - maintenance revision {#version-5.3.89}

- Fixed pagination during links merge
- Backported [from v6.3](v6-3#version-6.3.10) robustness of populate of nullable key to detect non-`null` empty values.
- Backported [from v6.3](v6-3#version-6.3.10) added `HTMLTool` methods variants to get HTML/Markdown resources contents with URLs substitutions.

### 5.3.90 (2026-06-19) - maintenance revision {#version-5.3.90}

- Backported [from v6.3](v6-3#version-6.3.11) fix on email regex to allow legit stacked dashes/plus signs
- Backported [from v6.3](v6-3#version-6.3.11) fix on custom action auto-save losing object data when retrying in an error state.

### 5.3.91 (UNRELEASED) - maintenance revision {#version-5.3.91}

- Backported [from v6.3](v6-3#version-6.3.12) fix on language inconsistency during the _forgot password_ procedure
