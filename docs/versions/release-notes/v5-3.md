...
- **Warning** (potential compatibility breaking change on legacy v4 code): the temporary "wrapper" classes `com.simplicite.util.PlatformHooksInterface`
  ...
  Make sure to refactor any outstanding code using the old fully qualified names and replace them with `com.simplicite.util.engine.PlatformHooksInterface`
