...
### 5.3.85 (UNRELEASED) - maintenance revision {#version-5.3.85}
...
- **Warning** (potential compatibility breaking change on legacy v4 code): the temporary "wrapper" classes `com.simplicite.util.PlatformHooksInterface`
  and its outdated counterpart `com.simplicite.util.GrantHooksInterface` that were there to ease the transition from v4 to v5 **have been finally removed**.
  Make sure to have refactored any outstanding code using the old fully qualified names and replace them with `com.simplicite.util.engine.PlatformHooksInterface`
  (and `com.simplicite.util.engineGrantHooksInterface` even if you should not be using the legacy grant hooks anymore but platform hooks).
