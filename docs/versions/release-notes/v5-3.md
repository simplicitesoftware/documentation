...
- Backported support for `MAIL_SERVICE` overriding by the same name environment variable (or `mail.service` JVM property)
  and backported support for YAML syntax instead of JSON

### 5.3.68 (UNDEFINED) - maintenance revision {#version-5.3.68}

- Backported default modules' Maven `skipTests` to `true` to prevent unit tests to be processed,
  by default, outside of a live instance
- Backported fix crosstab chart when grouping date per day (`dateGroup='D'`) when field is a datetime
