...
- Backported [from v6.2](v6-2#version-6.2.17) fix for issue where the list ordering preference was shown even when the object's configuration wasn't relevant (i.e. no field areas visible on the list)
- Backported [from v6.3](v6-3#other-technical-improvements) `Tool.before/after` date comparison shorthand methods

### 5.3.79 (UNRELEASED) - maintenance revision {#version-5.3.79}

- Filtered `com.simplicite.util.annotations` package from standard imports of Rhino scripts
  to avoid class name ambiguity errors (adding this package was required to allow Java class
  compilation outside of Simplicité, e.g. in an external IDE, when the code uses the annotations).
